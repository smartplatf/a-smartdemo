function Message() {
    ...
    this.deactivate = function(msg, cb) {
        this.callback = cb;
        this.submitSuccess = this.createcallback;
        var send = {};
        this.postDataTo(send, "DeActivateMessage", this.prime, msg);
    };
    ...
    this.listcallback = function(resp, smartobj) {
        ...
        if (resp instanceof smartresponse) {
            ...
            {
                smartobj.callback(lresp.list[i].values['message']);
            }
        }
    };
};

Message.prototype = new smart();


function createmessageconn()
{
    var fsmart = new Message();
